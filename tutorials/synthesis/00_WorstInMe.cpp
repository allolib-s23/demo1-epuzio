...
public:
  ...
  void onProcess(Graphics &g) override
  {
    ...
    g.color(frequency / 1000, frequency / 10, 200, 0.4);
    g.draw(mMesh);
    g.popMatrix();
    ...
    g.color(frequency / 10, frequency / 1000, 50, 0.4);
    g.draw(mMesh);
    g.popMatrix();
    ...
    g.color(frequency / 10, frequency / 10, frequency / 10, 0.4);
    g.draw(mMesh);
    g.popMatrix();
    ...
    g.pushMatrix();
    g.translate(cos(static_cast<double>(frequency)), sin(static_cast<double>(frequency)), -25);
    g.scale(frequency / 5000, 3 * frequency / 5000, frequency / 5000);
    g.color(frequency / 70, frequency / 70, frequency / 70, 0.4);
    g.draw(mMesh);
    g.popMatrix();
  }
...
public:
  ...
  virtual void onInit( ) override {
    ...
    void playVocals(){
        auto *voice = synthManager.synth().getVoice<Vocals>();
        synthManager.synthSequencer().addVoiceFromNow(voice, 0, 200);
    }

    // HELPER FUNCTIONS, CONSTS:
    float bpm = 135;
    //the actual bpm should be 125
    ...
    { // 52 measures of the same kick drum pattern
        for (int i = 0; i < 52; i++)
        {
            playKick(150, beatsElapsed(0 + (i * 4) + sequenceStart));
            playKick(300, beatsElapsed(1 + (i * 4) + sequenceStart));
            playKick(150, beatsElapsed(2 + (i * 4) + sequenceStart));
            playKick(150, beatsElapsed(2.5 + (i * 4) + sequenceStart));
            playKick(300, beatsElapsed(3 + (i * 4) + sequenceStart));
        }
    }

    void lfsrHiHat(int sequenceStart){
      srand(time(NULL));
    ...
    void strumPattern(int sequenceStart) //wet code, fix later?
    { 
        for (int i = 0; i < 13; i++)
        {
            for(int j = 0; j < 8; j++){
                playBass(getFreq("C", 3, 0), beatsElapsed(j/2.0 + (i * 16) + sequenceStart), eighth, .1);
                playBass(getFreq("E", 3, 0), beatsElapsed(j/2.0 + (i * 16) + sequenceStart), eighth, .1);
                playBass(getFreq("G", 3, 0), beatsElapsed(j/2.0 + (i * 16) + sequenceStart), eighth, .1);
            }
            for(int j = 0; j < 8; j++){
                playBass(getFreq("B", 2, 0), beatsElapsed(4 + j/2.0 + (i * 16) + sequenceStart), eighth, .1);
                playBass(getFreq("D", 3, 0), beatsElapsed(4 + j/2.0 + (i * 16) + sequenceStart), eighth, .1);
                playBass(getFreq("G", 3, 0), beatsElapsed(4 + j/2.0 + (i * 16) + sequenceStart), eighth, .1);
            }
            for(int j = 0; j < 7; j++){
                playBass(getFreq("D", 3, 0), beatsElapsed(8 + j/2.0 + (i * 16) + sequenceStart), eighth, .1);
                playBass(getFreq("B", 3, 0), beatsElapsed(8 + j/2.0 + (i * 16) + sequenceStart), eighth, .1);
            }

            playBass(getFreq("D", 3, 0), beatsElapsed(11 + (i * 16) + sequenceStart), quarter, .1);
            playBass(getFreq("D", 4, 0), beatsElapsed(11 + (i * 16) + sequenceStart), quarter, .1);

            for(int j = 0; j < 8; j++){
                playBass(getFreq("D", 3, 0), beatsElapsed(12 + j/2.0 + (i * 16) + sequenceStart), eighth, .1);
                playBass(getFreq("A", 3, 0), beatsElapsed(12 + j/2.0 + (i * 16) + sequenceStart), eighth, .1);
            }
        }
    }

    void chordWalk(int sequenceStart) //wet code, fix later?
    { 
      for (int i = 0; i < 11; i++)
      {
        playNote(getFreq("C", 3, 0), beatsElapsed((i * 16) + sequenceStart), quarter, .1);
        playNote(getFreq("E", 3, 0), beatsElapsed(1 + (i * 16) + sequenceStart), quarter, .1);
        playBass(getFreq("G", 3, 0), beatsElapsed(2 + (i * 16) + sequenceStart), half, .1);

        playNote(getFreq("B", 2, 0), beatsElapsed(4 + (i * 16) + sequenceStart), quarter, .1);
        playNote(getFreq("D", 3, 0), beatsElapsed(5 + (i * 16) + sequenceStart), quarter, .1);
        playNote(getFreq("G", 3, 0), beatsElapsed(6 + (i * 16) + sequenceStart), half, .1);

        playNote(getFreq("D", 3, 0), beatsElapsed(8 + (i * 16) + sequenceStart), quarter, .1);
        playNote(getFreq("B", 3, 0), beatsElapsed(9 + (i * 16) + sequenceStart), quarter, .1);
        playNote(getFreq("D", 4, 0), beatsElapsed(10 + (i * 16) + sequenceStart), half, .1);

        playNote(getFreq("D", 3, 0), beatsElapsed(12 + (i * 16) + sequenceStart), quarter, .1);
        playNote(getFreq("A", 3, 0), beatsElapsed(13 + (i * 16) + sequenceStart), quarter, .1);
        playNote(getFreq("A", 3, 0), beatsElapsed(14 + (i * 16) + sequenceStart), half, .1);
        }
    }

    void playTune()
    {
        //Song is 52 measures long
        playVocals(); //wav file with lyrics
        kickPattern(0);
        lfsrHiHat(0);
        strumPattern(0);
        chordWalk(32);
